/**
 * FLUX.1 Pro Image Generation Provider (via Replicate)
 * Fast, high-quality, flexible image generation
 */
...
export class FluxProProvider implements ImageProvider {
  ...
  async generate(params: ImageGenerationParams): Promise<GeneratedImageResult> {
    ...
    try {
      ...
      console.log('🎨 FLUX.1 Pro: Generating image...', {
      ...
      const output = await replicate.run(
        "black-forest-labs/flux-1.1-pro" as any,
        {
          input: {
            ...
            output_format: 'jpg',
            output_quality: 90,
            safety_tolerance: 2, // More permissive (allows "sexy", "attractive")
            prompt_upsampling: true, // Enhance the prompt automatically
          }
        }
      ) as any
      ...
      // FLUX.1 Pro cost: $0.025 per image
      const cost = 0.025
      ...
      if (!imageUrl) {
        throw new Error('No image URL returned from FLUX.1 Pro')
      }

      console.log('✅ FLUX.1 Pro: Image generated successfully', {
      ...
      return {
        url: imageUrl,
        revisedPrompt: params.prompt, // FLUX doesn't revise prompts
        width,
        height,
        format: 'jpg',
        provider: this.name,
      ...
      console.error('❌ FLUX.1 Pro generation error:', error)
      throw new Error(`FLUX.1 Pro generation failed: ${error.message}`)
    }
  }
}
